...
//for uploading score and user_name to db when game is over
function to_db(){
    // updating highscore sending data over to php
    if(update_highscore){
        let data2 = `highscore=${highscore_score}`;
        let req = new XMLHttpRequest();
        req.open('POST','start_game.php',true);
    ...
    let time_taken = Math.round((end_time-start_time)/1000);
    let minutes = "00";
    let seconds = "00";
    if(time_taken>=60){
        minutes = Math.round(time_taken/60);
        seconds = Math.round(time_taken%60);
        if(minutes<10)
            minutes = `0${minutes}`;
    }
    else
    ...
    if(seconds<10)
        seconds =  `0${seconds}`;
    // checking wether the user score is greater than before and checking wether the entered name is already in the database
    if(same_username_check){
        if(score>existing_score){
            let xhr = new XMLHttpRequest();
            let data = `bool=${same_username_check}&username=${username}&score=${score}&time=${minutes}:${seconds}`;
